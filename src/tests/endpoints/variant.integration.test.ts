...
describeIntegration("variant endpoint integration", () => {
  test("list returns variant rows", async () => {
    const endpoint = new VariantEndpoint(createIntegrationClient())

    const result = await endpoint.list({ pagination: { limit: 5, offset: 0 } })

    expect(Array.isArray(result.rows)).toBe(true)
    expect(result.meta.type).toBe("variant")
  })
